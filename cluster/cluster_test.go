...
var (
	mysqlCluster = mysqlv1alpha1.Cluster{
...
func TestEnsureVolumeClaimTemplates(t *testing.T) {
	...
	//when enable persistence
	{
		var cluster *Cluster
		storageClass := "ssd"
		testMysql := mysqlv1alpha1.Cluster{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "sample",
				Namespace: "default",
				Labels:    nil,
		...
		guard := PatchInstanceMethod(reflect.TypeOf(cluster), "GetLabels", func(*Cluster) labels.Set {
			return nil
		})
...
func TestEnsureMysqlConf(t *testing.T) {
	...
	testMysql := mysqlCluster
	testMysql.Spec = mysqlv1alpha1.ClusterSpec{
	...
	//cpu 8 corev1s,memory 16 gb,innodb_buffer_pool_size 2 gb
	{
		...
		testMysqlCase := testMysql
		testMysqlCase.Spec.PodSpec.Resources.Limits["cpu"] = *limitCpucorev1sCase
		testMysqlCase.Spec.MysqlOpts.Resources.Requests["memory"] = *memoryCase
		testCase := Cluster{
			&testMysqlCase,
		}
