...
运行如下指令，将显示名为 `mysqlusers.mysql.radondb.com` 的 CRD。
...
运行如下指令，将创建一个名为 `normal_user` 的普通用户和一个名为 `super_user` 的超级用户。用户密码保存在名为 `sample-user-password` 的 [Secret](https://kubernetes.io/docs/concepts/configuration/secret/) 中。
...
运行如下指令，使用 `super_user` 用户连接到 MySQL 集群主节点。
...
| tlsOptions.type           | TLS 类型；必须为 `NONE`/`SSL`/`X509`；默认为 `NONE` |
...
| secretSelector.secretKey  | 保存用户密码的 Secret 键名                  |
